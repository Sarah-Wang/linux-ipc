...
int main(int argc, char **argv)
{
	...
	// Create spooler
	r = pthread_create(&tid_spooler, NULL, spooler, NULL);
	if (r) {
	...
	for (i = 0; i < 10; i++) {
		thread_no[i] = i;
		r = pthread_create(&tid_producer[i], NULL,
			producer, (void *)&thread_no[i]);
		if (r) {
			fprintf(stderr, "Error = %d (%s)\n", r, strerror(r));
			exit(1);
		}
	...
	for (i = 0; i < 10; i++)
		r = pthread_join(tid_producer[i], NULL);
		if (r == -1) {
			fprintf(stderr, "Error = %d (%s)\n", r, strerror(r));
			exit(1);
		}
