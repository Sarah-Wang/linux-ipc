...
int main(int argc, char **argv)
{
	...
	qd_client = mq_open(client_queue_name,
			O_RDONLY | O_CREAT, QUEUE_PERMISSIONS, &attr);
	if (qd_client == -1) {
	...
	while (fgets(temp_buf, 2, stdin)) {
		...
		ret = mq_send(qd_server, client_queue_name,
				strlen(client_queue_name) + 1, 0);
		if (ret == -1) {
		...
		ret = mq_receive(qd_client, in_buffer, MSG_BUFFER_SIZE, NULL);
		if (ret == -1) {
